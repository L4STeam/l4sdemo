...
struct pie_params {
	...
	u32 ecndrop;		/* drop ecn if above this value */
};


#define TCA_PIE_ECNDROP TCA_PIE_BYTEMODE+1
#define __TCA_PIE_MAX TCA_PIE_BYTEMODE+2
...
#endif
{
	...
	if (!drop_early(sch, skb->len)) {
		enqueue = true;
	} else if (q->params.ecn && !(q->params.ecndrop && (q->vars.prob > q->params.ecndrop)) &&
		   INET_ECN_set_ce(skb)) {
		/* If packet is ecn capable, mark it if drop probability
		 * is lower than ecndrop parameter, else drop it.
		 */
		q->stats.ecn_mark++;
		enqueue = true;
	}
...
static int pie_change(struct Qdisc *sch, struct nlattr *opt)
{
	...
	if (tb[TCA_PIE_ECNDROP])
		q->params.ecndrop = (MAX_PROB / 100) * nla_get_u32(tb[TCA_PIE_ECNDROP]);
...
static int pie_dump(struct Qdisc *sch, struct sk_buff *skb)
{
	...
	if (nla_put_u32(skb, TCA_PIE_TARGET,
	    ...
	    nla_put_u32(skb, TCA_PIE_ECNDROP, q->params.ecndrop / (MAX_PROB / 100)))
		goto nla_put_failure;
