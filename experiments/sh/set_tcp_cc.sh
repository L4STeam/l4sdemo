...
if [ "$tcp_cc" == "cubic_ecn" ]; then
	...
	ssh $ip "sudo bash -c 'sudo echo 0 > /sys/module/tcp_prague/parameters/prague_rtt_scaling'"
elif [ "$tcp_cc" == "prague-additive" ]; then
	ssh $ip "sudo modprobe tcp_prague"
	ssh $ip "sudo bash -c 'sudo echo 3 > /sys/module/tcp_prague/parameters/prague_rtt_scaling'"
...
if [ "$ip" == "$SERVER_A" ]; then
	ssh $ip "sudo tc qdisc del root dev $SERVER_A_IFACE" || true
	ssh $ip "sudo tc qdisc replace root dev $SERVER_A_IFACE fq"
fi
if [ "$ip" == "$SERVER_B" ]; then
	ssh $ip "sudo tc qdisc del root dev $SERVER_B_IFACE" || true
	ssh $ip "sudo tc qdisc replace root dev $SERVER_B_IFACE fq"
fi
if [ "$ip" == "$CLIENT_A" ]; then
	ssh $ip "sudo tc qdisc del root dev $CLIENT_A_IFACE" || true
	ssh $ip "sudo tc qdisc replace root dev $CLIENT_A_IFACE fq"
fi
if [ "$ip" == "$CLIENT_B" ]; then
	ssh $ip "sudo tc qdisc del root dev $CLIENT_B_IFACE" || true
	ssh $ip "sudo tc qdisc replace root dev $CLIENT_B_IFACE fq"
fi
