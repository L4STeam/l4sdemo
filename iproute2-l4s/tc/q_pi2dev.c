...
{
	fprintf(stderr, "Usage: ... pi2dev [limit PACKETS] [target TIME us] [tupdate TIME us]\n");
...
static int pi2dev_parse_opt(struct qdisc_util *qu, int argc, char **argv,
			 struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "limit") == 0) {
			...
			if (get_float(&alpha_f, *argv) ||
			    (alpha_f > ALPHA_MAX) || (alpha_f < ALPHA_MIN)) {
				fprintf(stderr, "Illegal \"alpha\"\n");
				return -1;
			}
			alpha = (unsigned int)(alpha_f * 16);
		} else if (strcmp(*argv, "beta") == 0) {
			NEXT_ARG();
			if (get_float(&beta_f, *argv) ||
			    (beta_f > BETA_MAX) || (beta_f < BETA_MIN)) {
				fprintf(stderr, "Illegal \"beta\"\n");
				return -1;
			}
			beta = (unsigned int)(beta_f * 16);
...
static int pi2dev_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
...
static int pi2dev_print_xstats(struct qdisc_util *qu, FILE *f,
...
struct qdisc_util pi2dev_qdisc_util = {
	.id = "pi2dev",
	.parse_qopt	= pi2dev_parse_opt,
	.print_qopt	= pi2dev_print_opt,
	.print_xstats	= pi2dev_print_xstats,
};
