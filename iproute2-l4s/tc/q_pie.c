...
static void explain(void)
{
	...
	fprintf(stderr, "[beta BETA ][bytemode | nobytemode][ecn | noecn ][ecndrop PROBABILITY %%]\n");
}
			 ...
			 struct nlmsghdr *n)
{
	...
	while (argc > 0) {
		if (strcmp(*argv, "limit") == 0) {
			...
			ecn = 0;
		} else if (strcmp(*argv, "ecndrop") == 0) {
			NEXT_ARG();
			if (get_unsigned(&ecndrop, *argv, 0) ||
			    (ecndrop > P_MAX) || (ecndrop < P_MIN)) {
				fprintf(stderr, "Illegal \"ecndrop\"\n");
				return -1;
			}
	...
	if (bytemode != -1)
		addattr_l(n, 1024, TCA_PIE_BYTEMODE, &bytemode,
			  sizeof(bytemode));
	if (ecndrop)
		addattr_l(n, 1024, TCA_PIE_ECNDROP, &ecndrop,
			  sizeof(ecndrop));
...
static int pie_print_opt(struct qdisc_util *qu, FILE *f, struct rtattr *opt)
{
	...
	if (tb[TCA_PIE_ECNDROP] &&
	    RTA_PAYLOAD(tb[TCA_PIE_ECNDROP]) >= sizeof(__u32)) {
		ecndrop = rta_getattr_u32(tb[TCA_PIE_ECNDROP]);
		fprintf(f, "ecndrop %u ", ecndrop);
	}
