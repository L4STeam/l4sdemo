...
#define ENQ_STATES 3 // if higher, gives ramp that rises exponentially with #packets
#define MIN_ENQ 2
#define MAX_ENQ MIN_ENQ+ENQ_STATES-1
...
static u64 do_laqm(struct dualpi2_sched_data *q, struct sk_buff *skb,
			u64 now)
{
	u64 qdelay = 0;

	if (dualpi2_skb_cb(skb)->shortness_at_enq >= ENQ_STATES-1)
		return 0ULL;
	...
	if (qdelay >= q->step.thresh + (NSEC_PER_MSEC)) //max = min + range
		return (u64)MAX_PROB;

	if (qdelay > q->step.thresh) {
		qdelay -= q->step.thresh; //qdelay -= min
		qdelay *= MAX_PROB; //scale 0...MAX_PROB
		return div64_u64(qdelay, (NSEC_PER_MSEC)); // qdelay / range
	}

	return 0ULL;
}

static bool recur(struct dualpi2_sched_data *q, u32 p_L)
{
	q->count += p_L;
	if (q->count >= MAX_PROB) {
		q->count -= MAX_PROB;
...
static bool must_drop(struct Qdisc *sch, struct dualpi2_sched_data *q,
		      struct sk_buff *skb, u64 now)
{
	u64 local_l_prob, p_mL, p_L;
	...
	if (skb_is_l4s(skb)) {
		...
		p_mL = do_laqm(q, skb, now) >> dualpi2_skb_cb(skb)->shortness_at_enq; //p_'L == ramp a.k.a L4S Native AQM
		p_L = max(local_l_prob, p_mL); //local_l_prob == p_CL
		if (recur(q, (u32)p_L)) 
			goto mark;
				 ...
				 struct sk_buff **to_free)
{
	...
	if (q->drop_early && must_drop(sch, q, skb, 0)) {
	...
	if (skb_is_l4s(skb)) {
		...
		dualpi2_skb_cb(skb)->shortness_at_enq = MAX_ENQ - max(min(qdisc_qlen(q->l_queue), MAX_ENQ), MIN_ENQ);
...
pick_packet:
	...
	if (!q->drop_early && must_drop(sch, q, skb, now)) {
