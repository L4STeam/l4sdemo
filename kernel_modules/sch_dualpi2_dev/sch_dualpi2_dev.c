...
static void dualpi2_calculate_dequeue_credit_default(struct Qdisc *sch, struct dualpi2_sched_data *q)
{
	u32 mtu = psched_mtu(qdisc_dev(sch));
	q->vars.wL =  100 / q->params.dequeue_ratio - 1;
#ifdef WRR_OPT3
	q->vars.dequeue_credit_reset = (mtu * q->vars.wL);
#else
	q->vars.dequeue_credit_reset = (mtu * (wC - q->vars.wL));
#endif
	q->vars.dequeue_credit = q->vars.dequeue_credit_reset;
}
...
#endif
{
	...
	dualpi2_calculate_alpha_beta(q);
	dualpi2_calculate_dequeue_credit_default(sch, q);
...
#ifdef WRR_OPT3
/* Koen's version - opt3 */
static struct sk_buff *wrr_schedule(struct Qdisc *sch, struct dualpi2_sched_data *q,
			 struct sk_buff **skb_l, struct sk_buff **skb_c)
{
	if (*skb_l) {
		if (*skb_c) {
			if (q->vars.dequeue_credit >= q->vars.dequeue_credit_reset) { // over credits
				q->vars.dequeue_credit = 0; // reset credits. yield to C queue
				*skb_l = NULL;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
			...
			// else L queue builds credit
                        q->vars.dequeue_credit += qdisc_pkt_len(*skb_l);
			*skb_c = NULL;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
			return __skb_dequeue(&q->l_queue->q);
#else
			return __qdisc_dequeue_head(&q->l_queue->q);
#endif
		}
		// else only L, no C packets, reset credit
		q->vars.dequeue_credit = 0;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
		return __skb_dequeue(&q->l_queue->q);
#else
		return __qdisc_dequeue_head(&q->l_queue->q);
	...
	q->vars.dequeue_credit = 0; // if no L packets, always reset credits
	if (*skb_c) { // only C packets
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
                return __skb_dequeue(&sch->q);
#else
                return __qdisc_dequeue_head(&sch->q);
...
#else
/* Bob's version - opt1 (default) */
static struct sk_buff *wrr_schedule(struct Qdisc *sch, struct dualpi2_sched_data *q,
                         struct sk_buff **skb_l, struct sk_buff **skb_c)
{
         if (q->vars.dequeue_credit <= 0) {
                if (*skb_l) {
                        q->vars.dequeue_credit += qdisc_pkt_len(*skb_l);
                        *skb_c = NULL;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
			...
			*skb_l = NULL;
                ...
                if (*skb_c) {
                        q->vars.dequeue_credit -= qdisc_pkt_len(*skb_c) * q->vars.wL;
                        *skb_l = NULL;
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0)
                        return __skb_dequeue(&sch->q);
#else
        ...
        /* Reset the WRR credit as all queues are empty */
        q->vars.dequeue_credit = q->vars.dequeue_credit_reset;
        return NULL;
}
#endif
...
pick_packet:
	...
	skb = wrr_schedule(sch, q, &skb_l, &skb_c);
	now = ktime_get_ns();
	...
	if (skb_l) {
		...
		(qdelay_l >> 10 > q->params.ecn_thresh) && qlen_l > 1 :
